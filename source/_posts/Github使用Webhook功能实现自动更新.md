...
## 前言
{% note success %}
    是否还在为每次打打包代码后手动推送至服务器而烦恼
    现只需要在你的服务器轻轻的搭建一个接口，配合gihub的webhook，你的问题就迎刃而解
    每次编写完代码，只需要轻轻的点击一下git提交，你将会见证奇迹的发生
    话不多说让我们开始吧
{% endnote %}
...
{% note info %}
至此，接口部署成功，现在当你往仓库里推项目的时候你的网页会自动更新,但是这个项目还有几个问题，
*   项目接口返回慢，因为要全部完成才能返回状态码，基本上要3 ~ 4s
*   如果一个人重复推项目，当你第二次推项目的时候第一次还在打包或者一直重复推会导致项目崩溃，可能公司的项目也不会有人这样做，但是万一呢，你可以保证你自己，那其他人呢( 狗头保命 )
{% endnote %}

## Redis
为了解决以上两个问题，思来想去还是觉得使用异步队列方便一点，考虑了kafka、rabbitmq和redis还是觉得用Redis，因为redis部署简单，且我们自用项目也没有太多需要处理的消息，下面给大家看一张大佬发我的这几个的对比图
...
```
3. 优化，如果你的项目使用redis比较少可以将 订阅/发布写在一起，如果使用redis的地方比较多的话，我建议是将订阅单独拎出来，防止代码复杂化，若单独写的话你的订阅事件里包含了一个` deleteAndCloneRepository `方法，所以首先要把这个` deleteAndCloneRepository `方法单独写成一个js文件，(如果闲麻烦也可以在gihook.js里将这个方法放出来)
...
{% note info %}
设置redis后，上面的两个问题都被解决了，现在访问接口，耗时只有1.2s左右了，到这里项目就部署完毕，觉得对你有帮助的可以关注我，博客不定时更新，有问题可以在下方留言
{% endnote %}
